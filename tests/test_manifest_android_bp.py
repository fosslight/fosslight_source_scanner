...
def test_metadata_collector_skips_android_bp_license_extraction(tmp_path):
...
def test_source_item_marks_android_bp_at_creation():
    item = SourceItem("carrois-gothic-sc/Android.bp")
    assert item.is_manifest_file is True
...
def test_merge_results_sets_manifest_flag_without_overwriting_scancode_licenses():
    ...
    merged, _, _, _ = merge_results(scancode_result=[scancode_item])
